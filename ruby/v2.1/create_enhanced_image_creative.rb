...
#
# This example creates an enhanced image creative.
#
...
def create_enhanced_image_creative(dfareporting, args)
  util = CreativeAssetUtils.new(dfareporting, args[:profile_id])

  # Upload the first image asset
  image1_asset_id = util.uploadAsset(args[:advertiser_id],
      args[:path_to_image1_file], 'HTML_IMAGE')

  # Upload the second image asset
  image2_asset_id = util.uploadAsset(args[:advertiser_id],
      args[:path_to_image2_file], 'HTML_IMAGE')
  ...
  creative = {
    ...
    :creativeAssets => [
      {
        :assetIdentifier => image1_asset_id,
        :role => 'PRIMARY'
      },
      {
        :assetIdentifier => image2_asset_id,
        :role => 'PRIMARY'
      }
    ],
    :name => 'Example enhanced image creative',
  ...
  puts 'Created enhanced image creative with ID %d and name "%s".' %
...
if __FILE__ == $0
  ...
  args = DfaReportingUtils.get_arguments(
      ARGV, :profile_id, :advertiser_id, :size_id, :path_to_image1_file,
      :path_to_image2_file)

  # Authenticate and initialize API service
  dfareporting = DfaReportingUtils.setup()

  create_enhanced_image_creative(dfareporting, args)
end
