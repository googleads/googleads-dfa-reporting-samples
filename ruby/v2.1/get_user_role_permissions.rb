...
def get_user_role_permissions(dfareporting, args)
  # Construct and execute the subaccount request
  subaccount = dfareporting.subaccounts.get(
    :profileId => args[:profile_id],
    :id => args[:subaccount_id]
  ).execute()(

  # Construct the user role permissions request
  result = dfareporting.user_role_permissions.list(
    :profileId => args[:profile_id],
    :ids => subaccount.data.available_permission_ids
  ).execute()

  result.data.user_role_permissions.each do |permission|
...
if __FILE__ == $0
  ...
  get_user_role_permissions(dfareporting, args)
end
