...
def get_subaccounts(dfareporting, args)
  ...
  request = dfareporting.subaccounts.list(
  ...
  loop do
    ...
    # Display results
    result.data.subaccounts.each do |subaccount|
      puts 'Found subaccount with ID %d and name "%s".' %
          [subaccount.id, subaccount.name]
    end

    token = result.next_page_token

    break unless token and result.data.subaccounts.any?
