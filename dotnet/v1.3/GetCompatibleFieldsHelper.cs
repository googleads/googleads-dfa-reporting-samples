...
namespace DfaReporting.Sample
{
    ...
    internal class GetCompatibleFieldsHelper
    {
        ...
        /// <summary>
        /// Instantiate a helper for getting the compatible fields for a standard report.
        /// </summary>
        ...
        {
            Console.WriteLine("=================================================================");
            Console.WriteLine("Getting compatible fields for standard report with ID {0}",
                report.Id);
            Console.WriteLine("=================================================================");

            var compatibleFields = service.Reports.CompatibleFields.Query(report,
                  userProfileId).Execute();
            ...
            foreach (var dimension in standardReportCompatibleFields.Dimensions)
            {
              Console.WriteLine("Dimension \"{0}\" is compatible.", dimension.Name);
            }

            foreach (var metric in standardReportCompatibleFields.Metrics)
            {
              Console.WriteLine("Metric \"{0}\" is compatible.", metric.Name);
            }

            foreach (var dimensionFilter in standardReportCompatibleFields.DimensionFilters)
            {
              Console.WriteLine("Dimension Filter \"{0}\" is compatible.", dimensionFilter.Name);
            }

            foreach (var activityMetric in standardReportCompatibleFields.PivotedActivityMetrics)
            {
              Console.WriteLine("Pivoted Activity Metric \"{0}\" is compatible.",
                  activityMetric.Name);
            }
