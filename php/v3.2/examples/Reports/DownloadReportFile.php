...
class DownloadReportFile extends BaseExample {
  ...
  public function run() {
    ...
    if (!empty($fileId)) {
      printf('<h2>Retrieving contents of file %d for report %d</h2>',
          $fileId, $reportId);
      $this->flush();
      ...
      printf('<h2>No file found for profile ID %d and report ID %d</h2>',
          $userProfileId, $reportId);
    }
  }
  ...
  private function findFile($userProfileId, $reportId) {
    ...
    do {
      ...
      $response = $this->service->reports_files->listReportsFiles(
          $userProfileId, $reportId, ['pageToken' => $pageToken]);
  ...
  private function generateBrowserUrl($reportId, $fileId) {
    ...
    printf('<h3>Report file has browser URL: <a href="%s">%s</a></h3>',
