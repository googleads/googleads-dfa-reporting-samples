...
class GenerateReportFile extends BaseExample {
  ...
  public function run() {
    ...
    if (!empty($reportId)) {
      printf('<h2>Generating a report file for report with ID %d</h2>',
          $reportId);
      $this->flush();
  ...
  private function runReport($userProfileId, $reportId) {
    ...
    printf('Running report %d, current file status is %s<br>',
        $reportId, $file->getStatus());
    return $file;
  }
  ...
  private function waitForReportFile($reportId, $fileId) {
    ...
    do {
      ...
      if ($file->getStatus() === 'REPORT_AVAILABLE') {
        ...
        return null;
      } else if (time() - $startTime > self::MAX_RETRY_ELAPSED_TIME) {
        printf('File processing deadline exceeded<br>');
        return null;
      }

      $sleep = $this->getNextSleepInterval($sleep);
      printf('File status is %s, sleeping for %d seconds<br>',
          $file->getStatus(), $sleep);
      $this->sleep($sleep);
    } while (true);
  }
