...
class CreateRotationGroup extends BaseExample {
  ...
  public function run() {
    ...
    $creative_assignment->setCreativeId($values['creative_id']);
    ...
    $placement_assignment->setPlacementId($values['placement_id']);
    ...
    $delivery_schedule->SetPriority('AD_PRIORITY_01');
    ...
    $ad->setCreativeRotation($creative_rotation);
    $ad->setDeliverySchedule($delivery_schedule);
    $ad->setStartTime($startDate->format('Y-m-d') . 'T23:59:59Z');
    $ad->setEndTime($endDate->format('Y-m-d') . 'T00:00:00Z');
    $ad->setName($values['ad_name']);
    $ad->setPlacementAssignments(array($placement_assignment));
